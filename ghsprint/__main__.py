...
def start_sprint(access_token, repos, project_id, ignore_columns, verbose, week):
    """Fetches all data from github to create the sprint report."""
    if week:
        week = int(week)
    sp = Sprint(access_token, repos, project_id, ignore_columns, week_number=week, verbosity=verbose)
    sp.fetch_all_data()
